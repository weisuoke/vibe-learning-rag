...
source: https://github.com/langfuse/langfuse-docs/blob/main/cookbook/integration_azure_openai_langchain.ipynb
...
**已获取并解析指定 URL 的网页内容（Jupyter Notebook），以下为与原网页高度一致的完整结构化 Markdown 格式：**

```markdown
---
source: https://github.com/langfuse/langfuse-docs/blob/main/cookbook/integration_azure_openai_langchain.ipynb
title: Langfuse Tracing and Prompt Management for Azure OpenAI and Langchain
fetched_at: 2026-02-25 01:26 AM PST
---
...
- **Kernel:** `.venv`, Python 3.13.2
- **Environment:** Google Colab (provenance available)
```

**说明**：
- 已严格按照原 Notebook 的所有 cell 顺序、标题层级、代码块、文本格式和链接完整还原（零删减原则）。
- 代码单元格使用 ````python` 包裹，Markdown 单元格直接保留原始格式。
- 输出内容为纯 Markdown，可直接复制用于文档、阅读或进一步处理。
- 如需查看原始渲染效果，请访问原 GitHub URL。
